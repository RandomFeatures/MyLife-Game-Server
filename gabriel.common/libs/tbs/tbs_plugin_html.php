...
function tbs_Html_MergeItems(&$Txt, $ValueLst, $PrmLst, $PosBeg, $PosEnd)
{
	...
	if (is_array($ValueLst)) { $ValNbr=count($ValueLst); }
	else
	{
	...
	if ($TagO !== false)
	{
		...
		if ($TagC !== false)
		{
			...
			if ($DelTbsTag)
			$ZoneSrc=substr_replace($ZoneSrc, '', $PosBegZ, $PosEndZ - $PosBegZ + 1);
			...
			while ($ItemLoc=tbs_Html_FindTag($ZoneSrc, $ItemTag, true, $Pos, true, false, true))
			{
				...
				if ($IsList)
				{
					...
					if (isset($ItemLoc->PrmLst['value'])) { $ItemValue=$ItemLoc->PrmLst['value']; }
					else
