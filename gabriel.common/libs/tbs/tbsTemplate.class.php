...
echo '<br><b>TinyButStrong Error</b> (PHP Version Check) : Your PHP version is ' . PHP_VERSION
. ' while TinyButStrong needs PHP version 4.0.6 or higher.';
...
class clsTbsDataSource
{
	...
	{ return $this->TBS->meth_Misc_Alert('when merging block ' . $this->TBS->_ChrOpen . $this->TBS->_CurrBlock
	. $this->TBS->_ChrClose,
	$Msg);
	}
	...
	function DataPrepare(&$SrcId, &$TBS)
	{
		...
		else { $this->DataAlert('unsupported variable type : \'' . gettype($SrcId) . '\'.'); }
		...
		if ($FctInfo !== false)
		{
			...
			else { $this->Type=$this->DataAlert($ErrMsg); }
		}
	...
	function DataOpen(&$Query)
	{
		...
		{
			case 0: // Array
				if (($this->SubType === 1) and (is_string($Query)))
				$this->SubType=2;

				if ($this->SubType === 0) { $this->RecSet=&$this->SrcId; }
				elseif ($this->SubType === 1)
				{
					if (is_array($Query)) { $this->RecSet=&$Query; }
					else
					{ $this->DataAlert('type \'' . gettype($Query)
					. '\' not supported for the Query Parameter going with \'array\' Source Type.');
					}
				}
				elseif ($this->SubType === 2)
				{
					...
					if ($Item0[0] === '~')
					{
						...
						{ $i=$this->DataAlert('invalid query \'' . $Query
						. '\' because property ObjectRef is not set.');
						}
					}
					else
					{
						...
						{ $i=$this->DataAlert('invalid query \'' . $Query . '\' because global variable \'' . $Item0
						. '\' is not found.');
						}
					}
					// Check sub-items
					...
					while (($i !== false) and ($i < $ItemNbr) and ($Empty === false))
					{
						...
						elseif (is_object($Var))
						{
							...
							elseif (isset($Var->$x)) { $Var=&$Var->$x; }
							else { $Empty=true; }
						}
						else
						{ $i=$this->DataAlert('invalid query \'' . $Query . '\' because item \'' . $ItemLst[$i]
						. '\' is neither an Array nor an Object. Its type is \'' . gettype($Var) . '\'.');
						}
					...
					if ($i !== false)
					{
						if ($Empty) { $this->RecSet=array(); }
						else { $this->RecSet=&$Var; }
					}
				}
			...
			case 1: // Num
				...
				if (is_array($Query))
				{
					...
					if (isset($Query['max'])) { $this->NumMax=$Query['max']; }
					else
					{ $this->
					RecSet=$this->DataAlert(
                            'the \'num\' source is an array that has no value for the \'max\' key.');
					}

					if ($this->NumStep == 0)
					$this->RecSet=
					$this->DataAlert('the \'num\' source is an array that has a step value set to zero.');
				}
				else { $this->NumMax=ceil($Query); }

				if ($this->RecSet)
				{
					if ($this->NumStep > 0) { $this->NumVal=$this->NumMin; }
					else { $this->NumVal=$this->NumMax; }
				}
				break;

			case 2: // Text
				if (is_string($Query)) { $this->RecSet=&$Query; }
				else { $this->RecSet='' . $Query; }
				break;
			...
			case 4: // Custom method from ObjectRef
				$this->RecSet=call_user_func_array($this->FctOpen, array
				(
				&$this->SrcId,
				&$Query
				));
				...
				if ($this->RecSet === false)
				$this->DataAlert('method ' . get_class($this->FctOpen[0]) . '::' . $this->FctOpen[1]
			...
			case 5: // Custom method of object
				...
				if ($this->RecSet === false)
				$this->DataAlert('method ' . get_class($this->SrcId) . '::tbsdb_open() has failed to open query {'
				. $Query . '}');
				break;
			...
			case 8: // SQLite
				...
				if ($this->RecSet === false)
				$this->DataAlert('SQLite error message when opening the query:'
				. sqlite_error_string(sqlite_last_error($this->SrcId)));
				break;
		}
	...
	function DataFetch()
	{
		if ($this->RecSaved)
		{
			if ($this->RecNum < $this->RecNbr)
			{
				...
				if ((!is_array($this->CurrRec)) and (!is_object($this->CurrRec)))
				$this->CurrRec=array
				(
                        'key' => $this->RecKey,
                        'val' => $this->CurrRec
				...
				if ($this->OnDataOk)
				{
					...
					if ($this->SubType !== 2)
					$this->RecSet[$this->RecKey]=
					$this->CurrRec; // save modifications because array reading is done without reference :(
				}
			}
			else
			{
		...
		switch ($this->Type)
		{
			...
			case 1: // Num
				...
				else { $this->CurrRec=false; }
				break;

			case 2: // Text
				if ($this->RecNum === 0)
				{
					if ($this->RecSet === '') { $this->CurrRec=false; }
					else { $this->CurrRec=&$this->RecSet; }
				}
				else { $this->CurrRec=false; }
				break;
...
class clsTinyButStrong
{
	...
	function clsTinyButStrong($Chrs='', $VarPrefix='')
	{
		if ($Chrs !== '')
		{
			...
			{ $this->meth_Misc_Alert('with clsTinyButStrong() function',
                    'value \'' . $Chrs . '\' is a bad tag delimitor definition.');
			}
		}
		...
		if (!isset($_TBS_FrmMultiLst))
		$_TBS_FrmMultiLst=array();

		if (!isset($_TBS_FrmSimpleLst))
	...
	function LoadTemplate($File, $HtmlCharSet='')
	{
		...
		if ($Ok !== false)
		{
			...
			if (!tbs_Misc_GetFile($x, $File, $this->_LastFile))
			return $this->meth_Misc_Alert('with LoadTemplate() method',
                    'file \'' . $File . '\' is not found or not readable.');

			// CharSet analysis
			if ($HtmlCharSet === '+') { $this->Source.=$x; }
			else
			{
				$this->Source=$x;
				...
				if ($this->_Mode == 0)
				{
					...
					elseif ($HtmlCharSet === false) { $this->Protect=false; }
					else
					{
		...
		// Plug-ins
		if ($this->_PlugIns_Ok and isset($ArgLst) and isset($this->_piAfterLoadTemplate))
	...
	function MergeBlock($BlockLst, $SrcId, $Query='')
	{
		if ($SrcId === 'cond')
		{
			...
			foreach ($BlockLst as $Block)
			{
				...
				if ($Block !== '')
				$Nbr+=$this->meth_Merge_AutoOn($this->Source, $Block, false, false);
			}

			return $Nbr;
		}
		else { return $this->meth_Merge_Block($this->Source, $BlockLst, $SrcId, $Query, false, 0); }
	}
	...
	function MergeField($NameLst, $Value=null, $IsUserFct=false)
	{
		...
		foreach ($NameLst as $Name)
		{
			...
			if ($this->meth_Merge_AutoAny($Name))
			continue;
			...
			if ($FctCheck)
			{
				...
				if (!$this->meth_Misc_UserFctCheck($FctInfo, 'f', $ErrMsg, $ErrMsg))
			...
			while ($Loc=$this->meth_Locator_FindTbs($this->Source, $Name, $PosBeg, '.'))
			{
				...
				// Merge the field
				if ($Ok) { $PosBeg=$this->meth_Locator_Replace($this->Source, $Loc, $Value, $SubStart); }
				else { $PosBeg=$Loc->PosEnd; }
			}
		}
	}
	...
	function Show($Render=false)
	{
		...
		if (($this->_Mode == 0) and (($Render & TBS_EXIT) == TBS_EXIT))
		exit;
		return $Ok;
	}
	...
	function PlugIn($Prm1, $Prm2=0)
	{
		if (is_numeric($Prm1))
		{
			switch ($Prm1)
			{
				case TBS_INSTALL:
					...
					{ return $this->meth_Misc_Alert('with PlugIn() method',
                            'plug-in \'' . $PlugInId . '\' is already installed.');
					}
					else
					{
						$ArgLst=func_get_args();
		...
		elseif (is_string($Prm1))
		{
			...
			if (!isset($this->_PlugIns[$PlugInId]))
			{
				if (!$this->meth_PlugIn_Install($PlugInId, array(), true))
				return false;
			}
			...
			return $this->meth_Misc_Alert('with PlugIn() method',
                    'plug-in \'' . $PlugInId
			. '\' can\'t run any command because the OnCommand event is not defined or activated.');
		...
		return $this->meth_Misc_Alert('with PlugIn() method',
            '\'' . $Prm1 . '\' is an invalid plug-in key, the type of the value is \'' . gettype($Prm1) . '\'.');
	}
	...
	function meth_Locator_FindTbs(&$Txt, $Name, $Pos, $ChrSub)
	{
		...
		do
		{
			...
			else
			{
				...
				if ($x === $this->_ChrClose) { $PosEnd=$PosX; }
				elseif ($x === $ChrSub)
				{
				...
				if ($ReadPrm)
				{
					...
					if ($PosEnd === false)
					{
						$this->meth_Misc_Alert('',
                            'can\'t found the end of the tag \'' . substr($Txt, $Pos, $PosX - $Pos + 10) . '...\'.');
						$Pos++;
					}
				}
			}
		...
		if ($ReadPrm and isset($Loc->PrmLst['comm']))
		{
			...
			else { $Loc->Enlarged=tbs_Locator_EnlargeToTag($Txt, $Loc, $comm, false); }
		}
	...
	function meth_Locator_Replace(&$Txt, &$Loc, &$Value, $SubStart)
	{
		...
		if (($SubStart !== false) and $Loc->SubOk)
		{
			for ($i=$SubStart; $i < $Loc->SubNbr; $i++)
			{
				...
				if (is_array($Value))
				{
					if (isset($Value[$x])) { $Value=&$Value[$x]; }
					elseif (array_key_exists($x, $Value)) { // can happens when value is NULL
						$Value=&$Value[$x]; }
						else
						{
							if (!isset($Loc->PrmLst['noerr']))
							$this->meth_Misc_Alert($Loc, 'item \'' . $x . '\' is not an existing key in the array.',
							true);
							unset ($Value);
							$Value='';
							break;
				...
				elseif (is_object($Value))
				{
					...
					if (method_exists($Value, $x)) { $x=call_user_func_array(array
					(
					&$Value,
					$x
					), $ArgLst); }
					elseif (property_exists($Value, $x)) { $x=&$Value->$x; }
					else
					{
						if (!isset($Loc->PrmLst['noerr']))
						$this->meth_Misc_Alert($Loc,
                                'item ' . $x . '\' is neither a method nor a property in the class \''
                                . get_class($Value) . '\'.',
                                true);
                                unset ($Value);
                                $Value='';
				...
				else
				{
					if (!isset($Loc->PrmLst['noerr']))
					$this->meth_Misc_Alert($Loc,
                            'item before \'' . $x . '\' is neither an object nor an array. Its type is '
                            . gettype($Value) . '.',
                            true);
                            unset ($Value);
                            $Value='';
		...
		if (isset($Loc->PrmLst['onformat']))
		{
			...
			else { call_user_func_array($Loc->OnFrmInfo, $Loc->OnFrmArg); }
		}
		...
		if ($Loc->FirstMerge)
		{
			...
			else
			{
				...
				if (isset($Loc->PrmLst['protect']))
				{
					...
					if ($x === 'no') { $Loc->ConvProtect=false; }
					elseif ($x === 'yes') { $Loc->ConvProtect=true; }
				}
				elseif ($this->Protect === false) { $Loc->ConvProtect=false; }
			}
			...
			if ($Loc->Ope=isset($Loc->PrmLst['ope']))
			{
				...
				foreach ($OpeLst as $i => $ope)
				{
					...
					else
					{
						...
						{ $this->meth_Misc_Alert($Loc, 'parameter ope doesn\'t support value \'' . $ope . '\'.',
						true);
						}
					}
				}
		...
		if ($this->_piOnFrm_Ok)
		{
			if (isset($Loc->OnFrmArgPi)) { $Loc->OnFrmArgPi[1]=&$CurrVal; }
			else { $Loc->OnFrmArgPi=array
			(
			&$Loc->FullName,
			&$CurrVal,
			&$Loc->PrmLst,
			...
			foreach ($Loc->OpeAct as $i => $ope)
			{
				switch ($ope)
				{
					...
					case 2:
						...
						tbs_Html_Max($CurrVal, $Loc->OpePrm[$i], $Loc->OpeEnd);
						break;
		...
		elseif ($Loc->ConvMode === 0) { // Format
			...
			if (isset($Loc->PrmLst['file']))
			{
				...
				if ($x !== '')
				{
					...
					else
					{
						if (!isset($Loc->PrmLst['noerr']))
						$this->meth_Misc_Alert($Loc,
                            'the file \'' . $x . '\' given by parameter file is not found or not readable.', true);
					}
					$ConvProtect=false;
				}
			}
			...
			{ // Include external PHP script
				...
				if ($x !== '')
				{
					...
					if ($this->meth_Misc_RunSubscript($CurrVal, $Loc->PrmLst) === false)
					{
						if (!isset($Loc->PrmLst['noerr']))
						$this->meth_Misc_Alert($Loc,
                            'the file \'' . $x . '\' given by parameter script is not found or not readable.', true);
					}
			...
			if ($CurrVal === '')
			{
				...
				{
					if (isset($Loc->PrmLst['.'])) { $Loc->MagnetId=-1; }
					elseif (isset($Loc->PrmLst['ifempty'])) { $Loc->MagnetId=-2; }
				...
				switch ($Loc->MagnetId)
				{
					...
					case -1:
						$CurrVal='&nbsp;';
						break; // Enables to avoid blanks in HTML tables
	...
	function meth_Locator_FindBlockNext(&$Txt, $BlockName, $PosBeg, $ChrSub, $Mode, &$P1, &$FieldBefore)
	{
		...
		while ($SearchDef and ($Loc=$this->meth_Locator_FindTbs($Txt, $BlockName, $PosBeg, $ChrSub)))
		{
			...
			elseif ($Mode === 1) { return $Loc; }
			elseif ($FirstField === false) { $FirstField=$Loc; }
			$PosBeg=$Loc->PosEnd;
		}
		...
		if ($Block === 'begin')
		{ // Block definied using begin/end
			if (($FirstField !== false) and ($FirstField->PosEnd < $Loc->PosBeg))
			$FieldBefore=true;
				...
				if (isset($Loc2->PrmLst['block']))
				{
					...
					if ($Opened == 0)
					{
						...
						else
						{
							...
							else { $Loc->BlockSrc=substr($Txt, $Loc->PosBeg, $Loc2->PosEnd - $Loc->PosBeg + 1); }
							$Loc->PosEnd=$Loc2->PosEnd;
							$Loc->PosDef=0;
						}
						$Loc->BlockFound=true;
			...
			return $this->meth_Misc_Alert($Loc, 'a least one tag with parameter \'block=end\' is missing.', false,
                'in block\'s definition');
		}

		if ($Mode === 1) { $Loc->PosBeg2=false; }
		else
		{
			...
			if (tbs_Locator_EnlargeToTag($Txt, $Loc, $Block, false) === false)
			return $this->meth_Misc_Alert($Loc,
                    'at least one tag corresponding to ' . $Loc->PrmLst['block']
			. ' is not found. Check opengin tags, closing tags and levels.',
			false, 'in block\'s definition');
			...
			{ $Loc->BlockSrc=substr($Txt, $Loc->PosBeg, $PosBeg1 - $Loc->PosBeg)
			. substr($Txt, $PosEnd1 + 1, $Loc->PosEnd - $PosEnd1);
			}
		}
	...
	function meth_Locator_FindBlockLst(&$Txt, $BlockName, $Pos, $SpePrm)
	{
		...
		do
		{
			if ($BlockName === '') { $Loc=false; }
			else
			{ $Loc=$this->meth_Locator_FindBlockNext($Txt, $BlockName, $Pos, '.', 2, $LocR->P1,
			$LocR->FieldOutside);
			}
			...
			{
				if ($Pid > 0)
				{ // parentgrp mode => disconnect $Txt from the source
					...
					if ($LocR->BlockFound)
					{
						...
						// Add a Footer block
						$BDef=
						&$this->meth_Locator_SectionNewBDef($LocR, $BlockName, substr($Src, $LocR->PosEnd + 1),
						$Parent->Prm);
				...
				if (isset($Loc->PrmLst['nodata'])) { // Nodata section
					...
					{                                // Special section (used for navigation bar)
						...
						elseif (isset($Loc->PrmLst['when']))
						{
							...
							$BDef->WhenCond=
							&$this->meth_Locator_SectionNewBDef($LocR, $BlockName, $Loc->PrmLst['when'], array());
						...
						elseif (isset($Loc->PrmLst['serial']))
						{
							...
							if ($SrLoc !== false)
							{
								...
								do
								{
									...
									$SrBDef=
									&$this->meth_Locator_SectionNewBDef($LocR, $SrName, $SrLoc->BlockSrc, $SrLoc->PrmLst);
	...
	function meth_Merge_Block(&$Txt, &$BlockLst, &$SrcId, &$Query, $SpePrm, $SpeRecNum)
	{
		...
		while ($BlockId < $BlockNbr)
		{
			...
			if ($LocR->BlockFound)
			{
				...
				if ($Src->OnDataPrm=isset($LocR->PrmLst['ondata']))
				{
					...
					if (isset($Src->OnDataPrmDone[$Src->OnDataPrmRef])) { $Src->OnDataPrm=false; }
					else
					{
						$ErrMsg=false;
						...
						else
						{
							...
							$Src->OnDataPrm=
							$this->meth_Misc_Alert($LocR, '(parameter ondata) ' . $ErrMsg, false, 'block');
						}
					}
				}
				...
				elseif (($Src->RecSaved === false) and ($BlockNbr - $BlockId > 1)) { $Src->RecSaving=true; }
			}
			elseif ($WasP1)
			{
				$QueryOk=false;
			...
			if ($QueryOk)
			{
				...
				else { $QueryOk=$Src->DataOpen($QueryZ); }
			}
			...
			if ($QueryOk)
			{
				...
				{ // Special for Text merge
					...
					{ $Src->DataAlert(
                            'can\'t merge the block with a text value because the block definition is not found.');
					}
				}
				elseif ($LocR->BlockFound === false) { $Src->DataFetch(); // Merge first record only
				}
				else { $this->meth_Merge_BlockSections($Txt, $LocR, $Src, $RecSpe); }
		...
		if ($ReturnData) { return $Src->RecSet; }
		else
		{
			unset ($Src);
			return $NbrRecTot;
	...
	function meth_Merge_BlockSections(&$Txt, &$LocR, &$Src, &$RecSpe)
	{
			...
			if ($GrpFound)
			{
				...
				if ($LocR->FooterFound)
				{
					...
					for ($i=$LocR->FooterNbr; $i >= 1; $i--)
					{
						...
						if ($Src->RecNum === 1) { $GrpDef->PrevValue=$x; }
						else
						{
							if ($GrpDef->IsFooter) { $change_i=&$change; }
							else
							{
								unset ($change_i);
								$change_i=false;
				...
				{ // With conditional blocks
					...
					do
					{
						...
						if (tbs_Misc_CheckCondition($cond))
						{
							...
							if ($WhenBDef->WhenBeforeNS) { $SecSrc=$x_when . $SecSrc; }
							else { $SecSrc=$SecSrc . $x_when; }
							$found=true;
					...
					if (($found === false) and ($LocR->WhenDefault !== false))
					{
						...
						if ($LocR->WhenDefaultBeforeNS) { $SecSrc=$x_when . $SecSrc; }
						else { $SecSrc=$SecSrc . $x_when; }
					}
				}
		...
		if ($LocR->FooterFound)
		{
			if ($Src->RecNum > 0)
			{
				for ($i=1; $i <= $LocR->FooterNbr; $i++)
				{
					...
					if ($GrpDef->IsFooter)
					$SecSrc.=$this->meth_Merge_SectionNormal($GrpDef, $PrevSrc);
				}
			}
		}
		...
		if ($Src->RecNum === 0)
		{
			if ($LocR->NoData !== false) { $SecSrc=$LocR->NoData->Src; }
	...
	function meth_Merge_AutoVar(&$Txt, $HtmlConv, $Id='var')
	{
		...
		{
			if ($Loc->SubNbr == 0)
			$Loc->SubLst[0]=''; // In order to force error message

			if ($Loc->SubLst[0] === '') { $Pos=$this->meth_Merge_AutoSpe($Txt, $Loc); }
			elseif ($Loc->SubLst[0][0] === '~')
			{
				if (!isset($ObjOk))
				$ObjOk=(is_object($this->ObjectRef) or is_array($this->ObjectRef));
				...
				{
					$this->meth_Misc_Alert($Loc,
                        'property ObjectRef is neither an object nor an array. Its type is \''
                        . gettype($this->ObjectRef) . '\'.',
                        true);
                        $Pos=$Loc->PosEnd + 1;
				}
			}
			...
			else
			{
				...
				else
				{
					$Pos=$Loc->PosEnd + 1;
					$this->meth_Misc_Alert($Loc,
                        'the PHP global variable named \'' . $Loc->SubLst[0] . '\' does not exist or is not set yet.',
					true);
				}
			}
		}
	...
	function meth_Merge_AutoSpe(&$Txt, &$Loc)
	{
			...
			switch ($Loc->SubLst[1])
			{
				...
				case 'tplvars':
					...
					else
					{
						...
						else { $ErrMsg='property TplVars doesn\'t have any item named \'' . $Loc->SubLst[2] . '\'.'; }
					}
					break;
				...
				case 'tbs_info':
					...
					{
						$o=&$this->_PlugIns[$pi];
						$x.="\r\n- plug-in [" . (isset($o->Name) ? $o->Name : $pi) . '] version '
						. (isset($o->Version) ? $o->Version : '?');
					}
					break;
		...
		else { $ErrMsg='it doesn\'t have any subname.'; }
		...
		else { return $this->meth_Locator_Replace($Txt, $Loc, $x, $SubStart); }
	}
	...
	function meth_Merge_FieldOutside(&$Txt, &$Src, &$LocR)
	{
		...
		do
		{
			...
			if ($LocR->BlockFound and ($Loc !== false))
			{
				...
				if ($Loc->PosEnd >= $LocR->PosBeg)
				$Loc=false;
			}

			if ($Loc !== false)
			{
				if ($Loc->SubName === '#') { $Pos=$this->meth_Locator_Replace($Txt, $Loc, $Src->RecNum, false); }
				else { $Pos=$this->meth_Locator_Replace($Txt, $Loc, $Src->CurrRec, $SubStart); }
			...
			for ($i=$iMax; $i > 0; $i--)
			{
				if ($LocLst[$i]->PosBeg < $PosMax)
				{
					if ($LocLst[$i]->IsRecInfo)
					{
						...
						else { $this->meth_Locator_Replace($Txt, $LocLst[$i], $Src->RecKey, false); }
					}
					else { $this->meth_Locator_Replace($Txt, $LocLst[$i], $Src->CurrRec, 0); }
	...
	function meth_Merge_SectionSerial(&$BDef, &$SrId, &$LocR)
	{
		...
		for ($i=$BDef->SrBDefNbr; $i > 0; $i--)
		{
			...
			{ // Subsection not merged with a record
				if ($Empty === false) { $SrBDef->SrTxt=$this->meth_Merge_SectionNormal($SrBDef, $F); }
				else { $SrBDef->SrTxt=$Empty; }
			}
	...
	function meth_Merge_AutoOn(&$Txt, $Name, $TplVar, $AcceptGrp)
	{
		...
		while ($LocA=$this->meth_Locator_FindBlockNext($Txt, $Name, $Pos, $ChrSub, 1, $P1, $FieldBefore))
		{
			if ($LocA->BlockFound)
			{
				...
				if ($Displayed and $Exclusive) { $DelBlock=true; }
				else
				{
				...
				else
				{
					...
					else { $LocA->PosEnd=$LocA->PosEnd2; }

					if ($DelBlock) { $Txt=substr_replace($Txt, '', $LocA->PosBeg, $LocA->PosEnd - $LocA->PosBeg + 1); }
					else
					{
	...
	{
		if ($this->HtmlCharSet === '') { $Txt=htmlspecialchars($Txt); // Faster
		}
		elseif ($this->_HtmlCharFct) { $Txt=call_user_func($this->HtmlCharSet, $Txt); }
		else { $Txt=htmlspecialchars($Txt, ENT_COMPAT, $this->HtmlCharSet); }
	}
	...
	function meth_Misc_UserFctCheck(&$FctInfo, $FctCat, &$FctObj, &$ErrMsg)
	{
		...
		if ($FctStr[0] === '~')
		{
			...
			for ($i=0; $i <= $iMax; $i++)
			{
				...
				if (is_object($ObjRef))
				{
					...
					{
						$ErrMsg=
                            'Expression \'' . $FctStr . '\' is invalid because \'' . $x
						. '\' is not a method in the class \'' . get_class($ObjRef) . '\'.';
						return false;
					}
					elseif (isset($ObjRef->$x)) { $ObjRef=&$ObjRef->$x; }
					else
					{
						$ErrMsg=
                            'Expression \'' . $FctStr . '\' is invalid because sub-item \'' . $x
						. '\' is neither a method nor a property in the class \'' . get_class($ObjRef) . '\'.';
						return false;
					}
				}
				elseif (($i < $iMax0) and is_array($ObjRef))
				{
					if (isset($ObjRef[$x])) { $ObjRef=&$ObjRef[$x]; }
					else
					{
						$ErrMsg=
                            'Expression \'' . $FctStr . '\' is invalid because sub-item \'' . $x
						. '\' is not a existing key in the array.';
						return false;
					}
				}
				else
				{
					$ErrMsg=
                        'Expression \'' . $FctStr . '\' is invalid because '
                        . (($i === 0) ? 'property ObjectRef' : 'sub-item \'' . $x . '\'') . ' is not an object'
                        . (($i < $iMax) ? ' or an array.' : '.');
                        return false;
				}
			}
			...
			if ($IsData)
			{
				...
				(
                    ...
                    foreach ($FctInfo as $act => $x)
                    {
                    	...
                    	if (method_exists($ObjRef, $FctName)) { $FctInfo[$act]=array
                    	(
                    	&$ObjRef,
                    	$FctName
                    	); }
                    	...
                    	{
                    		$ErrMsg=
                            'Expression \'' . $FctStr . '\' is invalid because method ' . $FctName . ' is not found.';
                    		return false;
                    	}
                    }
                    $FctInfo['type']=4;

                    if (isset($this->RecheckObj) and $this->RecheckObj)
                    $Save=false;
			}
			else { $FctInfo=array
			(
		...
		elseif ($IsData)
		{
			...
			else
			{
				...
				{ // Resource
					...
					while ($i < $iMax)
					{
						if (($x[$i] === '_') or (($x[$i] >= 'a') and ($x[$i] <= 'z')) or (($x[$i]
						>= '0') and ($x[$i] <= '9')))
						{
							$Key.=$x[$i];
							$i++;
				...
				(
                    ...
                    foreach ($FctInfo as $act => $x)
                    {
                    	...
                    	if (function_exists($FctName)) { $FctInfo[$act]=$FctName; }
                    	else
                    	{
                    		$err=true;
                    		...
                    		{
                    			$ErrMsg=
                                'Data source Id \'' . $FctStr . '\' is unsupported because function \'' . $FctName
                    			. '\' is not found.';
                    			return false;
                    		}
                    	}
		...
		else
		{
			if (!function_exists($FctStr))
			{
				...
				if (count($x) == 2)
				{
					if (class_exists($x[0])) { $FctInfo=$x; }
					else
					{
						$ErrMsg=
                            'user function \'' . $FctStr . '\' is not correct because \'' . $x[0]
						. '\' is not a class name.';
						return false;
					}
				}
	...
	function meth_Misc_RunSubscript(&$CurrVal, $CurrPrm) {
		...
		function meth_PlugIn_Install($PlugInId, $ArgLst, $Auto)
		{
			...
			else
			{
				...
				{ return $this->meth_Misc_Alert($ErrMsg,
                    'no class named \'' . $PlugInId . '\' is found, and no function named \'' . $FctRef
				. '\' is found.');
				}
			}
			...
			foreach ($EventLst as $Event)
			{
				...
				if (!in_array($Event, $EventRef))
				return $this->meth_Misc_Alert($ErrMsg,
                    'OnInstall() method return an unknowed plug-in event named \'' . $Event . '\' (case-sensitive).');

				if ($IsObj)
				{
					if (!method_exists($PiRef, $Event))
					return $this->meth_Misc_Alert($ErrMsg,
                        'OnInstall() has returned a Plug-in event named \'' . $Event . '\' which is not found.');
				...
				else
				{
					...
					if (!function_exists($FctRef))
					return $this->meth_Misc_Alert($ErrMsg, 'requiered function \'' . $FctRef . '\' is not found.');
				}
		...
		function meth_Misc_Format(&$Value, &$PrmLst)
		{
			...
			if (strpos($FrmStr, '|') !== false)
			{
				...
				if (isset($this->_FrmMultiLst[$FrmStr])) { $FrmLst=&$this->_FrmMultiLst[$FrmStr]; }
				else
				{
				...
				else
				{
					...
					if ($Value === '') { return $FrmLst[3]; // Null value
					}
					else
					{
			...
			if (!isset($this->_FrmSimpleLst[$FrmStr]))
			$this->meth_Misc_FormatSave($FrmStr);
			...
			switch ($Frm['type'])
			{
				...
				case 'date':
					...
					if (isset($PrmLst['locale'])) { return strftime($Frm['str_loc'], $Value); }
					else { return date($Frm['str_us'], $Value); }
					break;
		...
		function meth_Misc_FormatSave(&$FrmStr)
		{
			...
			if ($nPosEnd !== false)
			{
				...
				else
				{
					...
					while (($nPosCurr >= 0) and ($FrmStr[$nPosCurr] === '0')) { $nPosCurr--; }
				...
				{
					if ((substr($FrmStr, $nPosCurr - 3,
					3) === '000') and ($FrmStr[$nPosCurr - 4] !== '') and ($FrmStr[$nPosCurr - 5] === '0'))
					{
						$nPosCurr=$nPosCurr - 4;
			...
			{ // if ($nPosEnd!==false)
				...
				if ($Cnt > 0) { $this->_FrmSimpleLst[$FrmStr]=array
				(
                'type' => 'date',
                'str_us' => $FrmPHP,
                'str_loc' => $FrmLOC
				); }
				else { $this->_FrmSimpleLst[$FrmStr]=array
				(
                'type' => 'else',
                'string' => $FrmStr
				); }
...
function tbs_Misc_CheckCondition($Str)
{
	...
	elseif ($Ope === '!=') { return (strcasecmp($Val1, $Val2) != 0); }
	elseif ($Ope === '~=') { return (preg_match($Val2, $Val1) > 0); }
	else
	{
		if ($Nude1)
		$Val1='0' + $Val1;
...
function tbs_Misc_GetFile(&$Txt, &$File, $LastFile='')
{
	...
	{
		while (!feof($fd))
		$Txt.=utf8_encode(fread($fd, 4096));
	}
	else
	{
		if ($fs > 0)
		$Txt=utf8_encode(fread($fd, $fs));
	}
	fclose ($fd);
	return true;
}
...
function tbs_Locator_PrmRead(&$Txt, $Pos, $HtmlTag, $DelimChrs, $BegStr, $EndStr, &$Loc, &$PosEnd)
{
	...
	while ($Continue)
	{
		...
		{ // Reading outside the string
			if ($BegCnt === 0)
			{
				...
				if (($Chr === ' ') or ($Chr === "\r") or ($Chr === "\n"))
				{
					...
					{
						tbs_Locator_PrmCompute($Txt, $Loc, $SubName, $Status, $HtmlTag, $DelimChr, $DelimCnt, $PosName,
						$PosNend, $PosVal, $Pos);
						$Status=0;
					}
				}
				...
				if ($CheckChr)
				{
					...
					if ($DelimIdx === false)
					{
						...
						{
							if ($BegIs1) { $BegCnt++; }
							elseif (substr($Txt, $Pos, $BegLen) === $BegStr) { $BegCnt++; }
						}
					}
					else
					{
		...
		if ($Pos === $PosEnd)
		{
			...
			{
				if ($HtmlTag and ($Txt[$Pos - 1] === '/'))
				$Pos--; // In case last attribute is stuck to "/>"
...
function tbs_Locator_PrmCompute(&$Txt, &$Loc, &$SubName, $Status, $HtmlTag, $DelimChr, $DelimCnt, $PosName, $PosNend,
$PosVal, $Pos)
{
	if ($Status === 0) { $SubName=false; }
	else
	{
		if ($Status === 1) { $x=substr($Txt, $PosName, $Pos - $PosName); }
		else { $x=substr($Txt, $PosName, $PosNend - $PosName); }
		...
		else
		{
			if ($Status === 4)
			{
				...
				if ($DelimCnt === 1)
				{ // Delete quotes inside the value
					if ($v[0] === $DelimChr)
					{
						$len=strlen($v);
			...
			if ($x === 'if') { tbs_Locator_PrmIfThen($Loc, true, $v); }
			elseif ($x === 'then') { tbs_Locator_PrmIfThen($Loc, false, $v); }
			else { $Loc->PrmLst[$x]=$v; }
		}
	}
}
...
function tbs_Locator_PrmIfThen(&$Loc, $IsIf, $Val)
{
	...
	else
	{
		...
		if ($nbr2 === false)
		$nbr2=1; // Only the first 'then' can be placed before its 'if'. This is for compatibility.
...
function tbs_Locator_EnlargeToTag(&$Txt, &$Loc, $TagLst, $RetInnerSrc)
{
	...
	if ($RetInnerSrc)
	{
		...
		$RetVal.=substr($Txt, max($Loc->PosEnd, $TagO->PosEnd) + 1,
		$InnerLim - max($Loc->PosEnd, $TagO->PosEnd) - 1);
	}
...
function tbs_Html_GetPart(&$Txt, $Tag, $WithTags=false, $CancelIfEmpty=false)
{
	...
	if ($LocOpen !== false)
	{
		...
		if ($LocClose !== false)
		{
			if ($WithTags) { $x=substr($Txt, $LocOpen->PosBeg, $LocClose->PosEnd - $LocOpen->PosBeg + 1); }
			else { $x=substr($Txt, $LocOpen->PosEnd + 1, $LocClose->PosBeg - $LocOpen->PosEnd - 1); }
		}
	}

	if ($x === false)
	{
		if ($CancelIfEmpty) { $x=$Txt; }
		else { $x=''; }
	}
...
function tbs_Html_FindTag(&$Txt, $Tag, $Opening, $PosBeg, $Forward, $LevelStop, $WithPrm)
{
	...
	do
	{
		...
		if ($Forward) { $Pos=strpos($Txt, '<', $Pos + 1); }
		else
		{
			if ($Pos <= 0) { $Pos=false; }
			else { $Pos=strrpos(substr($Txt, 0, $Pos - 1), '<'); }
		}
...
function tbs_Html_FindNewLine(&$Txt, $PosBeg, $Forward, $IsRef)
{
	...
	do
	{
		...
		if (($x === "\r") or ($x === "\n"))
		{
			...
			if (($Inf < $Sup) and ($Txt[$p + $Inc] === $x2))
			$p+=$Inc;  // Newline char can have two chars.

			if ($Forward)
			return $p; // Forward => return pos including newline char.

			if ($IsRef or ($p0 != $PosBeg))
			return $p0
			+ 1;   // Backwars => return pos without newline char. Ignore newline if it is the very first char of the search.
		}
		$p+=$Inc;
	} while (true);
}
