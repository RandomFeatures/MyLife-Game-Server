...
DROP PROCEDURE IF EXISTS `mylife_player`.`dsp_Player_Start` GO
CREATE DEFINER=`mylife_game`@`%` PROCEDURE `mylife_player`.`dsp_Player_Start`(
	kvUserID int,	
...
BEGIN
	...
	IF lcount = 0 THEN
		...
		SELECT 
				...
				kvXRef
		FROM 
			mylife_player.tbl_Player_Start
		LIMIT 1;
	END IF;
END;
GO
...
DROP PROCEDURE IF EXISTS `mylife_player`.`dsp_Player_Pending_Collect` GO
CREATE DEFINER=`mylife_game`@`%` PROCEDURE `mylife_player`.`dsp_Player_Pending_Collect`(
kvUserID int,
kvIDKey varchar(20))
BEGIN
	DECLARE lcount INT;	
	...
	-- see if the player already got the gift
	SELECT 
		count(1) 
	INTO 
		lcount
	FROM
		mylife_player.tbl_Player_Gift_Recieved
	WHERE
		UserID = kvUserID
	AND
		PendingID = @pendingID
	...
	IF lcount = 0 THEN
		-- record that the gift was recieved
		INSERT INTO mylife_player.tbl_Player_Gift_Recieved(
				UserID, 
				PendingID, 
				Recieved) 
		VALUES(
...
DROP PROCEDURE IF EXISTS `mylife_player`.`dsp_Player_Pending_Complete` GO
CREATE DEFINER=`mylife_game`@`%` PROCEDURE `mylife_player`.`dsp_Player_Pending_Complete`(
	kvUserID int,
	kvIDKey varchar(20))
BEGIN
	DECLARE lcount INT;
...
DROP PROCEDURE IF EXISTS `mylife_player`.`dsp_Player_Requests_upd` GO
CREATE DEFINER=`mylife_game`@`%` PROCEDURE `mylife_player`.`dsp_Player_Requests_upd`(
kvUserID int,
kvIDList mediumtext)
BEGIN
	DECLARE lcount INT;
	...
	INTO 
		lcount
	FROM 
		mylife_player.tbl_Player_Requests_Sent
	WHERE 
		UserID = kvUserID
	AND
		LastSent = DATE(NOW());	
	...
	IF lcount = 0 THEN
		INSERT INTO mylife_player.tbl_Player_Requests_Sent(
				UserID, 
				LastSent, 
				FriendsList) 
		VALUES (
	...
	ELSE
		UPDATE 
			mylife_player.tbl_Player_Requests_Sent 
		SET  
			FriendsList = FriendsList + kvIDList
		WHERE 
			UserID = kvUserID
		AND
			LastSent = DATE(NOW());
...
DROP PROCEDURE IF EXISTS `mylife_player`.`dsp_Player_Requests_sel` GO
CREATE DEFINER=`mylife_game`@`%` PROCEDURE `mylife_player`.`dsp_Player_Requests_sel`(
kvUserID int)
BEGIN
	SELECT id, UserID, LastSent, FriendsList 
	FROM mylife_player.tbl_Player_Requests_Sent
	WHERE
		UserID = kvUserID
	AND
		LastSent = DATE(NOW());	
